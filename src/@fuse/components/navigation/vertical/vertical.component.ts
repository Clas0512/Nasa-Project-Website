...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnChanges(changes: SimpleChanges)
    {
        ...
        if ( 'navigation' in changes )
        {
            ...
            // changes["navigation"].currentValue.forEach(async (element:FuseNavigationItem , index )  => {
            //      let perm = element.meta
         
            //      if (perm) {
            //          //console.log(1)
            //   //   this.checkPerm(perm ,  element , false)
            //             //console.log(3)
            //      }

            //      if (element.children) {
                  
            //        await  element.children.forEach(async (item ,childIndex) =>{
              

            //             if (item.meta) {
            //                 const childPerm = item.meta
            //                 let navIndex =  this.navigation.findIndex(t => t.id === element.id)
            //             //    this.checkPerm(childPerm ,element, true ,childIndex)
            //             }
